<template>
  <div class="page">
    <div class="page-header">
      <div>
        <h1>Notes</h1>
        <p>{{ store.notes.length }} notes in your workspace</p>
      </div>
    ...
    <div class="filters">
      ...
      <input v-model="noteDateFilter" type="date" class="filter-select" />
      <input v-model="startDateFilter" type="date" class="filter-select" />
      <input v-model="endDateFilter" type="date" class="filter-select" />
      <input v-model="search" type="text" class="search-input" placeholder="Search note title…" />
    </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-if="filteredNotes.length === 0">
              <td colspan="7" class="empty-row">No notes found.</td>
            </tr>
            <tr v-for="(note, index) in filteredNotes" :key="note.id">
              ...
              <td>{{ note.noteDate || '—' }}</td>
              <td class="description-cell">{{ note.content || '—' }}</td>
              <td class="actions">
                <button class="action-btn edit" @click="openModal(note)">Edit</button>
                <button class="action-btn delete" @click="confirmDelete(note)">Delete</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
        <div class="modal">
          ...
          <form class="modal-form" @submit.prevent="saveNote">
            <div class="field">
            ...
            <div class="field">
              <label>Content</label>
              <textarea v-model="form.content" rows="6" placeholder="Write your note"></textarea>
            </div>
            <div class="field">
              <label>Note Date</label>
              <input v-model="form.noteDate" type="date" />
            </div>
            <div v-if="apiError" class="api-error">{{ apiError }}</div>
            <div class="modal-actions">
              <button type="submit" class="btn-primary">{{ editingNote ? 'Save Changes' : 'Add Note' }}</button>
    ...
    <Teleport to="body">
      <div v-if="deleteTarget" class="modal-overlay" @click.self="deleteTarget = null">
        <div class="modal modal-sm">
          ...
          <div class="modal-actions">
            <button class="btn-danger" @click="deleteNote">Yes, Delete</button>
...
watch([courseFilter, noteDateFilter, startDateFilter, endDateFilter], loadNotes)
...
const filteredNotes = computed(() => {
  ...
  return store.notes.filter((note) =>
    ...
    note.departmentName.toLowerCase().includes(query) ||
...
async function loadNotes() {
  await store.fetchNotes({
    ...
    noteDate: noteDateFilter.value || undefined,
...
async function deleteNote() {
  ...
  try {
    await store.deleteNote(deleteTarget.value.id)
...
.description-cell { min-width: 260px; color: #6b7280; white-space: pre-wrap; }
